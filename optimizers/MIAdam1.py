...
class MIAdam1(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(MIAdam1, self).__init__(params, defaults)
        
        self.buf_exp_avgs = []

    def __setstate__(self, state):
        super(MIAdam1, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)

            
    @staticmethod
    def miadam1(params: List[Tensor],
             grads: List[Tensor],
        ...
        for i, param in enumerate(params):
            ...
            kappa1=kappa
            if epoch<sub_epoch:
                step_size = (lr**2) / bias_correction1
                buf_exp_avg.mul_(kappa1).add_(exp_avg)
                param.addcdiv_(buf_exp_avg, denom, value=-step_size)
    ...
    def step(self, closure=None,epoch=0,kappa=0,sub_epoch=0):
        ...
        for group in self.param_groups:
            ...
            MIAdam1.miadam1(params_with_grad,
                               grads,
                               exp_avgs,
                               self.buf_exp_avgs,
                               exp_avg_sqs,
