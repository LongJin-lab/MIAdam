...
class MIAdam2(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(MIAdam2, self).__init__(params, defaults)
        
        self.buf_exp_avgs = []
        self.buf_exp_avgs2 = []

    def __setstate__(self, state):
        super(MIAdam2, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)

            
    @staticmethod
    def miadam2(params: List[Tensor],
             grads: List[Tensor],
        ...
        for i, param in enumerate(params):
            ...
            # print("buf_exp_avg=", buf_exp_avg)
            kappa1=kappa
            if epoch<sub_epoch:
                step_size = (lr**3) / bias_correction1
                buf_exp_avg.mul_(kappa1).add_(exp_avg)
                buf_exp_avg2.mul_(kappa1).add_(buf_exp_avg)
                param.addcdiv_(buf_exp_avg2, denom, value=-step_size)
            else :
    ...
    def step(self, closure=None,epoch=0,kappa=0,sub_epoch=0):
        ...
        for group in self.param_groups:
            ...
            MIAdam2.miadam2(params_with_grad,
                               grads,
                               exp_avgs,
                               self.buf_exp_avgs,
                               self.buf_exp_avgs2,
