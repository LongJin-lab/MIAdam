...
def add_noise_to_targets(train_set, noise_frac, dataset='CIFAR10'):
    num_labels = 10
    num_samples = len(train_set.targets)
    num_noisy_samples = int(noise_frac * num_samples)
    noisy_idx = list(range(num_samples))
    np.random.shuffle(noisy_idx)
    noisy_idx = noisy_idx[0:num_noisy_samples]
    train_set.targets = np.array(train_set.targets)
    train_set.targets[noisy_idx] = np.random.randint(0, num_labels, num_noisy_samples)
...
if __name__ == '__main__':
    ...
    #add noise
    add_noise_to_targets(trainset,noise_factor , 'CIFAR10')
    ...
    net = selectarch(args.arch, num_classes=10)
    net = net.to(device)
